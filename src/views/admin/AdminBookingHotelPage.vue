<template>
  <div class="admin-bookinghotel-page">
    <h1 class="animate-pulse">📅 Quản lý Đặt lịch</h1>
    <p class="subtitle">Danh sách các lịch đặt khách sạn từ khách hàng</p>
    ...
    <div class="filter-box">
      <div class="form-row">
        <label for="check_in_date">Ngày vào:</label>
        <input type="date" id="check_in_date" v-model="filter.check_in" />
      </div>
      <div class="form-row">
        <label for="check_out_date">Ngày ra:</label>
        <input type="date" id="check_out_date" v-model="filter.check_out" />
      </div>
      <div class="form-row">
    ...
    <div class="bookinghotel-list">
      <div class="bookinghotel-card" v-for="(bookinghotel, index) in bookinghotels" :key="index">
        <h3><strong>Tên khách hàng:</strong> {{ bookinghotel.name }}</h3>
        <p><strong>Email:</strong> {{ bookinghotel.email }}</p>
        <p><strong>Số điện thoại:</strong> {{ bookinghotel.phone }}</p>
        <p><strong>Ngày:</strong> Check-in {{ formatDate(bookinghotel.check_in) }} | Check-out {{ formatDate(bookinghotel.check_out) }}</p>
        <p><strong>Giờ:</strong> Check-in {{ bookinghotel.check_in_time }} | Check-out {{ bookinghotel.check_out_time }}</p>
        ...
        <p>
          ...
          <span :style="{ color: bookinghotel.handled ? '#43a047' : '#ff6b6b' }">
...
export default {
  ...
  methods: {
    async fetchbookinghotel() {
      const params = {};
      if (this.filter.check_in) params.check_in = this.filter.check_in;
      if (this.filter.check_out) params.check_out = this.filter.check_out;
      if (this.filter.handled !== '') params.handled = this.filter.handled;

      try {
        const res = await apiClient.get('http://localhost:8000/api/bookinghotel', { params });
    ...
    resetFilter() {
      this.filter.check_in = '';
      this.filter.check_out = '';
      this.filter.handled = '';
      this.fetchbookinghotel();
    },
    formatDate(date) {
...
.admin-bookinghotel-page::before {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
