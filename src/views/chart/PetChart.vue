<template>
  <div class="pet-chart">
    <h1 class="animate-pulse">� Thống kê Thú cưng</h1>
    <p class="subtitle">Thống kê tỷ lệ các loài thú cưng (Chó, Mèo, v.v.)</p>
    <div class="chart-container">
      <div id="pet_pie_chart" style="width: 100%; height: 600px"></div>
    </div>
  </div>
</template>
...
const fetchTypeChart = async () => {
  try {
    ...
    const chartData = [['Type', 'Total']]
    categories.forEach(c => {
      chartData.push([c.type, c.total])
    })

    const data = window.google.visualization.arrayToDataTable(chartData)
    const options = {
      title: 'Tỷ lệ các loài thú cưng',
      is3D: true,
    ...
    const chart = new window.google.visualization.PieChart(
      document.getElementById('pet_pie_chart')
    )
...
onMounted(() => {
  ...
  script.onload = () => {
    ...
    window.google.charts.setOnLoadCallback(fetchTypeChart)
  }
